...
export async function searchPlacesByVibe(destination: string, vibes: string[], originLat = 0, originLng = 0): Promise<PlaceResult[]> {
  try {
    const keywords = vibes.slice(0, 3).join(' ');
    const query = `${keywords} in ${destination}`;
    const url = `${BASE}/textsearch/json?query=${encodeURIComponent(query)}&key=${API_KEY}`;
    console.log('Google Places URL:', url);
    const res = await fetch(url);
    const data = await res.json();
    console.log('Google Places status:', data.status);
    console.log('Google Places results count:', data.results?.length);
